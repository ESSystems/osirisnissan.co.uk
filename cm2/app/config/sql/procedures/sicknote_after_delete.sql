DROP TRIGGER IF EXISTS sicknote_after_delete;

DELIMITER | 

CREATE TRIGGER sicknote_after_delete AFTER DELETE ON sicknotes
	FOR EACH ROW BEGIN
		...
		SELECT MIN(S.start_date), MAX(S.end_date), SUM(sick_days)
		  ...
		  FROM sicknotes S
		 WHERE S.absence_id = OLD.absence_id;
